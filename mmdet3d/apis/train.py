...
def train_detector(model,
    ...
    # build runner
    # if 'frozen_bevdep4d' in cfg:
    #     params = list()
    #     for name, p in model.named_parameters():
    #         if 'radar_bbox_head' in name:
    #             params.append(p)    
    #         else:
    #             layer = getattr(model, name.split('.', 1)[0])
    #             layer.eval()  # 保证动量等不会更新
    #     optimizer = torch.optim.AdamW(params, lr=cfg.optimizer.lr, weight_decay=cfg.optimizer.weight_decay)  # todo: 目前只根据训练时使用adamw进行直接修改，若在config中切换其他optim则此处需要修改
    # else:
    optimizer = build_optimizer(model, cfg.optimizer)
    ...
    if validate:
        ...
        val_dataloader = build_mmdet_dataloader(
            ...
            workers_per_gpu=min(cfg.data.workers_per_gpu, 2),
            dist=distributed,
            shuffle=False)
        eval_cfg = cfg.get('evaluation', {})
        eval_cfg['by_epoch'] = cfg.runner['type'] != 'IterBasedRunner'
